...
function show(req, res) {
  ...
  const moviesSql = `
    SELECT 
        m.*, ROUND(AVG(r.vote)) AS average_vote
    FROM
        movies AS m
            LEFT JOIN
        reviews AS r ON m.id = r.movie_id
    WHERE
        m.id = ?`;
  const reviewSql = `
    SELECT 
  ...
  sqlConnect.query(moviesSql, [id], (err, movieResult) => {
    if (err)
      return res.status(500).json({ error: "Database query error (Movie)" }); // catch error
